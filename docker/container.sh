...
{
    set -x
    docker exec -it ${container} /bin/bash ${regress_dir}/CantianKernel/docker/init_container.sh ${user} ${core_dir} ${commitID}
    set +x
}
...
function startnode()
{
    ...
    container="cantian_${mode}-node${node_id}"
...
function init()
{
    ...
    echo "mysql_source: ${mysql_source}"
    echo "mounts: ${mounts}"
}
