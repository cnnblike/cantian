...
MYSQL_CODE_PATH=${WORKSPACE}/cantian-connector-mysql/mysql-source
...
func_version()
{
    ...
    echo "name=\"Mysql 100\"" >> ${CANTIANDB_BIN}/package.xml
    echo "version=\"${VERSION_MYSQLIP} ${BUILD_MODE}\"" >> ${CANTIANDB_BIN}/package.xml
    echo "desc=\"Mysql 100 install\"" >> ${CANTIANDB_BIN}/package.xml
...
func_make_mysql_debug()
{
  ...
  if [ "${MYSQL_BUILD_MODE}" == "multiple" ]; then
    if [ "${ENABLE_LLT_GCOV}" == "YES" ]; then
      ...
      cmake .. -DWITH_TSE_STORAGE_ENGINE=${WITH_TSE_STORAGE_ENGINE} -DCMAKE_BUILD_TYPE=Debug -DWITH_BOOST=${BOOST_PATH} -DWITHOUT_SERVER=OFF -DCMAKE_CXX_COMPILER=${CMAKE_CXX_COMPILER} -DCMAKE_C_COMPILER=${CMAKE_C_COMPILER}
    fi
  elif [ "${MYSQL_BUILD_MODE}" == "single" ]; then
    cmake .. -DWITH_DAAC=1 -DWITH_TSE_STORAGE_ENGINE=${WITH_TSE_STORAGE_ENGINE} -DCMAKE_BUILD_TYPE=Debug -DWITH_BOOST=${BOOST_PATH} -DWITHOUT_SERVER=OFF -DCMAKE_CXX_COMPILER=${CMAKE_CXX_COMPILER} -DCMAKE_C_COMPILER=${CMAKE_C_COMPILER}
  fi
