...
-- -- 获取锁的事务信息：DATA_LOCKS
CREATE OR REPLACE VIEW cantian_locks AS SELECT SID, TYPE, ID1, ID2, LMODE, BLOCK, RMID FROM `cantian`.`dv_locks`;
-- -- 锁等待事务信息监控：DATA_LOCKS_WAITS
CREATE OR REPLACE VIEW cantian_lock_waits AS SELECT `cantian`.`dv_locks`.TYPE , `cantian`.`dv_locks`.ID1, `cantian`.`dv_locks`.ID2, `cantian`.`dv_locks`.LMODE, `cantian`.`dv_locks`.BLOCK, `cantian`.`dv_locks`.RMID AS locks_RMID,  `cantian`.`dv_transactions`.* FROM `cantian`.`dv_locks` JOIN `cantian`.`dv_transactions` ON `cantian`.`dv_transactions`.SID = `cantian`.`dv_locks`.SID JOIN `cantian`.`dv_sessions` ON `cantian`.`dv_transactions`.SID = `cantian`.`dv_sessions`.SID
WHERE `cantian`.`dv_sessions`.LOCK_WAIT = 'Y';
-- -- 超过10s/60s/180s/600s的事务个数：DV_TRANSACTIONS
CREATE OR REPLACE VIEW cantian_transactions AS SELECT * FROM ( ( SELECT COUNT(*) AS GT10s_CNT FROM `cantian`.`dv_transactions` WHERE EXEC_TIME > (10 * 1000000) ) AS t1, ( SELECT COUNT(*) AS GT60s_CNT FROM `cantian`.`dv_transactions` WHERE EXEC_TIME > (60 * 1000000) ) AS t2, ( SELECT COUNT(*) AS GT180s_CNT FROM `cantian`.`dv_transactions` WHERE EXEC_TIME > (180 * 1000000) ) AS t3, ( SELECT COUNT(*) AS GT600s_CNT FROM `cantian`.`dv_transactions` WHERE EXEC_TIME > (600 * 1000000) ) AS t4 );
-- -- DV_BUFFER_RECYCLE_STATS
CREATE OR REPLACE VIEW cantian_buffer_recycle_stats AS SELECT * FROM `cantian`.`dv_buffer_recycle_stats`;
-- -- buffer pool命中率：DV_BUFFER_ACCESS_STATS
CREATE OR REPLACE VIEW cantian_buffer_access_stats AS SELECT * FROM `cantian`.`dv_buffer_access_stats`;
-- --fsync data当前等待数频率：DV_SYS_STATS
CREATE OR REPLACE VIEW cantian_data_pending_fsyncs AS SELECT * FROM `cantian`.`dv_sys_stats` WHERE NAME = 'DBWR disk writes';
-- -- fsync log当前等待数频率：DV_SYS_STATS
CREATE OR REPLACE VIEW cantian_os_log_pending_fsyncs AS SELECT * FROM `cantian`.`dv_sys_stats` WHERE NAME = 'redo wirtes';
-- -- Redo Log Pending Writes会话级日志写操作被挂起的次数：DV_SESSION_EVENTS
CREATE OR REPLACE VIEW cantian_redo_log_pending_writes_session AS SELECT * FROM `cantian`.`dv_session_events` WHERE EVENT = 'log file sync';
-- -- Redo Log Pending Writes系统级日志写操作被挂起次数：DV_SYS_EVENTS
CREATE OR REPLACE VIEW cantian_redo_log_pending_writes_sys AS SELECT * FROM `cantian`.`dv_sys_events` WHERE EVENT = 'log file sync';
