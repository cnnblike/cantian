...
{
    knl_session_t *session = (knl_session_t *)g_rc_ctx->session;
    CT_LOG_RUN_INF("[ARCH_STANDBY] is primary %d, is dbstor %d, is master %d, is changed %d", 
        knl_db_is_primary(session), cm_dbs_is_enable_dbs(), rc_is_master(), g_rc_ctx->info.master_changed);
    if (knl_db_is_primary(session) || !cm_dbs_is_enable_dbs()) {
