...
typedef struct st_dtc_rcy_node {
    ...
    bool32 recover_done;
    bool32 recover_read_done;
    bool32 recover_read_done_after_update_index_cnt;
...
#define DTC_RCY_GET_CURR_BATCH(dtc_rcy, idx , index) \
    ((log_batch_t *)((dtc_rcy)->rcy_nodes[(idx)].read_buf[(index)].aligned_buf + (dtc_rcy)->rcy_nodes[(idx)].read_pos[(index)]))
