...
class MysqlShell:
    ...
    def start_session(self, timeout=10):
        cmd = [self.mysql_cmd, '-u', self.user]
        if self.password:
            cmd.append('-p')
        else:
    ...
    def _read_output(self, timeout=None):
        ...
        while True:
            ...
            try:
                if self.master_fd in select.select([self.master_fd], [], [], timeout)[0]:
...
def lock_instance_for_backup():
    ...
    try:
        mysql_shell.execute_command("SET @ctc_ddl_enabled=true", timeout=3)
        output = mysql_shell.execute_command("LOCK INSTANCE FOR BACKUP", timeout=3)
        print("LOCK INSTANCE FOR BACKUP output: ", output)
    except TimeoutError as e:
